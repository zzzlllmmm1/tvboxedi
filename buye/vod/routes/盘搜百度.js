...
const CONFIG = {
  // 校验服务地址，为空则跳过校验
  CHECK_LINK_API: "http://192.168.50.50:7024/api/v1/links/check", // 例如: "http://192.168.50.50:7024/api/v1/links/check"
  // 其他可配置项
  CHECK_LINK_TIMEOUT: 20000,
  VALID_STATUS: ["valid_links"], // ["valid_links", "pending_links"]
  DEFAULT_PIC: "https://xget.xi-xu.me/gh/power721/alist-tvbox/raw/refs/heads/master/web-ui/public/baidu.jpg"
};
...
const search = async ({ page, quick, wd }) => {
  ...
  for (const key in ret.data.merged_by_type || {}) {
    ...
    const pic = drive?.pic || CONFIG.DEFAULT_PIC;
  ...
  // 4. 调用链接校验接口 - 只有配置了地址才执行校验逻辑
  let validLinksSet = new Set();
  
  // 判断是否配置了校验服务地址
  if (CONFIG.CHECK_LINK_API && CONFIG.CHECK_LINK_API.trim()) {
    try {
      const checkRes = await _http.post(
        CONFIG.CHECK_LINK_API, // 使用配置的校验服务地址
        {
          links: uniqueLinks,
          selected_platforms: [
            "baidu"
        ...
        {
          timeout: CONFIG.CHECK_LINK_TIMEOUT,
          headers: {
      ...
      for (const status of CONFIG.VALID_STATUS) {
...
const store = {
  ...
  meta: {
    ...
    name: "盘搜|百度",
    type: 4,
    api: "/video/pansou-baidu",
    searchable: 1,
    quickSearch: 1,
    changeable: 0,
  },
  home: async ({ filter }) => ({
    class: [{ type_id: 1, type_name: "Pure Search|No Resources" }],
    list: [],
  }),
  category: async ({ id, page, filter, filters }) => ({
    list: [{
      vod_id: 1,
      vod_name: "No Resources",
      vod_pic: "",
      vod_remarks: "Pure search",
    }],
    page: 1,
    pagecount: 1,
  }),
